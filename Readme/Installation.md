...
предварительное форматирование преамбулы диссертации (может потребоваться установка пакета`texlive-mylatexformat` ). Это позволяет
...
```
Для преодоления такого препятствия в менеджере пакетов который вам больше нравиться ищите impnattypo
В результатах поиска будет пакет содержащий данный стиль, этот пакет нужно поставить стандартным способом. После этого, сообщения:
```
File `impnattypo.sty' not found. ^^M
```
...
* Лучше всего использовать актуальные и полные версии LaTeX-дистрибутивов, это поможет избежать многих проблем. Например, [MikTeX](http://miktex.org/download) 2.9.6361+ для Windows или [TeXLive](http://www.tug.org/texlive/acquire.html) 2017+ для множества ОС.
...
make spell-check
```
По умолчанию проверка будет осуществлена с использованием русского словаря в файлах папок `Dissertation`, `Presentation` и `Synopsis`.
...
make compress
```
Сжатый файл будет создан с именем `dissertation_compressed.pdf`
...
```
make compress COMPRESSION_FILE=Synopsis.pdf
```
