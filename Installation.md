# Установка и [сборка](Installation#Сборка-pdf)
...
Кроме того, есть упрощённый вариант шаблона (черновик) в файле
`draft.tex` (пока только для автореферата), который требует в
...
при наборе формул. Для него в каталоге `Synopsis` доступны следующие
команды:
